...
class Html5 extends CHtml {
    ...
    public static function imLink($text, $to, $htmlOptions=array()){
        return self::ajaxLink($text, Yii::app()->createUrl('im/popup'), 
            array(
            	'type' => 'post', 
            	'dataType' => 'json',
                'data' => "js:{'to': '{$to}'}",
            	'success' => "js:function(json){
                    switch(parseInt(json.status, 10)){
                        case 0:	//失败
                            alert(String(json.msg));
                            break;
                        case 2:	//不在线
                            alert('您处于离线状态，无法进行即时聊天');
                            break;
                    }
    			}",
            ),
